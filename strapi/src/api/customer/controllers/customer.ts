...
export default {
  register: async (ctx, next) => {
    try {
      ...
      if (!order) return ctx.forbidden('No order exists')
     ...
     let user =  await strapi.db.query('plugin::users-permissions.user').findOne({
      ...
      if (!user && data.existingAccount) return ctx.NotFound('No user with this email address was found. Register with a different address.')


      // If user found but not delcared an existing account
      if (user && !data.existingAccount) return ctx.MethodNotAllowed('A user account with this address already exists. Supply a new email or select option for existing account')
