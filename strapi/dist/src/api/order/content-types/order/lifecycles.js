module.exports = {
    async afterUpdate(event) {
        ...
        if (!result.custom_data)
            return console.log('Attempted completion data failed: No custom data attached to this order');
        //The order must already have an attached user record
        if (!((_a = result.user) === null || _a === void 0 ? void 0 : _a.id))
            return console.log('Attempted completion data failed: No user is linked to this order');
        ...
        if (!course || !course.chapters)
            return console.log('Attempted completion data failed: Either no course or no course chapters found');
        ...
        if (existing)
            return console.log('Attempted completion data failed: a user-course-progress record already exists for this user and course.');
        // Preparing all of the completion data to be stored as data
        const chapterCourseCompletion = course.chapters.map(crs => {
        ...
        const subchapterCourseCompletion = subchapters.map(sub => {
