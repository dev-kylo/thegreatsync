...
const PageSteps = ({ currIndex, pageSteps, showNextButton, type, nextPage, prevPage, nextStep, prevStep, goToStep }: PageStepsProps) => {

    const stepsContainer = useRef<HTMLOListElement>(null);

    useEffect(() => {
        function scrollIntoView(indx: number) {
            if (stepsContainer.current) stepsContainer.current.querySelector(`#step_${indx}`)!.scrollIntoView({ behavior: "smooth" })
        }
